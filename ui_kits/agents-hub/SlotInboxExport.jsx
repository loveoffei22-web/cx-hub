...
const ExportDay = ({agent, tickets}) => {
  const wat = new Date(new Date().toLocaleString('en-US',{timeZone:'Africa/Lagos'}));
  const todayStr = wat.toISOString().slice(0,10);

  // Detect the actual date range covered by the tickets
  const MONTHS_LONG = ['January','February','March','April','May','June','July',
                       'August','September','October','November','December'];
  const uniqueDates = [...new Set(
    tickets.map(t => t.created?.slice(0,10)).filter(Boolean)
  )].sort();

  let periodDay, periodDayLong, fileDate;
  if (uniqueDates.length === 0) {
    // No tickets — fall back to today
    periodDay     = todayStr;
    periodDayLong = wat.toLocaleDateString('en-GB', {weekday:'long', day:'numeric', month:'long', year:'numeric'});
    fileDate      = todayStr;
  } else if (uniqueDates.length === 1) {
    const [y, m, d] = uniqueDates[0].split('-').map(Number);
    const date = new Date(y, m - 1, d);
    periodDay     = uniqueDates[0];
    periodDayLong = date.toLocaleDateString('en-GB', {weekday:'long', day:'numeric', month:'long', year:'numeric'});
    fileDate      = uniqueDates[0];
  } else {
    const [fy, fm, fd] = uniqueDates[0].split('-').map(Number);
    const [ly, lm, ld] = uniqueDates[uniqueDates.length - 1].split('-').map(Number);
    const firstDate = new Date(fy, fm - 1, fd);
    const lastDate  = new Date(ly, lm - 1, ld);
    periodDay = `${uniqueDates[0]} – ${uniqueDates[uniqueDates.length - 1]}`;
    if (fm === lm) {
      // Same month: "18–22 May 2026"
      periodDayLong = `${fd}–${ld} ${MONTHS_LONG[lm - 1]} ${ly}`;
    } else {
      // Cross-month: "30 Apr – 2 May 2026"
      periodDayLong = `${fd} ${MONTHS_LONG[fm - 1]} – ${ld} ${MONTHS_LONG[lm - 1]} ${ly}`;
    }
    fileDate = `${uniqueDates[0]}_${uniqueDates[uniqueDates.length - 1]}`;
  }

  const dateStr = fileDate; // keep for filename display below

  const payload = {
    exportedAt: new Date().toISOString(),
    exportedBy: {name: agent.name, email: agent.email, id: agent.id},
    period: {day: periodDay, dayLong: periodDayLong},
    tickets,
    stats: {
  ...
  const download = () => {
    ...
    a.download = `ekedp_${agent.id}_${fileDate}.json`;
    a.click();
