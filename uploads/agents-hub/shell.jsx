...
// Tinted with the active agent's colour throughout.
...
const Shell = ({agent, active, onChange, onSignOut, children}) => {
  ...
  return (
    <div style={{minHeight:'100vh', background:'#060E2B', color:'#F0F4FF', fontFamily:'Sora,sans-serif'}}>
      <nav style={{
        ...
        <div onClick={()=>setCollapsed(c=>!c)} style={{
          ...
          {!collapsed && (
            <div>
              <div style={{font:'800 11px/1.1 Sora,sans-serif', color:'#fff', letterSpacing:'.2px'}}>EKEDP POWER APP</div>
              <div style={{font:'500 9px/1.2 Sora,sans-serif', color:'#6B85C0'}}>Agents Hub</div>
            </div>
          )}
        </div>
        ...
        {!collapsed && (
          <div style={{
            ...
            borderBottom:'1px solid #1E3070',
            display:'flex', alignItems:'center', gap:10,
          }}>
            {photos[agent.id] ? (
              <img src={photos[agent.id]} alt={agent.name}
                style={{
            ...
            <div style={{minWidth:0}}>
              <div style={{font:'italic 500 9.5px/1 Sora,sans-serif', color:'#6B85C0'}}>{greet} {greetE}</div>
              <div style={{font:'800 13px/1.1 Sora,sans-serif', color:'#fff', marginTop:3, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{agent.name.split(' ')[0]}</div>
            </div>
          </div>
        )}
        ...
        <div style={{display:'flex', flexDirection:'column', gap:5, paddingTop:10, borderTop:'1px solid #1E3070'}}>
          {!collapsed && (
            <div style={{fontFamily:'JetBrains Mono,monospace', fontSize:10, color:'#6B85C0', padding:'0 6px', marginBottom:4, textAlign:'center'}}>
              {watTime} WAT · {watDate}
            </div>
          )}
      ...
      <main style={{paddingLeft: W, transition:'padding-left .25s ease', minHeight:'100vh'}}>
        {/* Topbar with agent colour accent */}
        <div style={{
          background:'linear-gradient(135deg, ' + agent.color + '11, transparent 30%), #0D1B45',
          ...
          <div style={{font:'italic 800 18px/1.2 Sora,sans-serif', color:'#fff', marginTop:4}}>
